...
def api_employers():
    return {"employers": bot_core.load_employers()}

@app.post("/api/employers")
def api_add_employer(data: dict):
    name = data.get("name", "").strip()
    admin_id = data.get("admin_id", "").strip()
    if len(name) < 2 or len(admin_id) < 1:
        raise HTTPException(400, "نام و آیدی کارفرما الزامی است")
    emp = bot_core.add_employer(name, admin_id, data.get("description", ""))
    return {"ok": True, "employer": emp}

@app.put("/api/employers/{emp_id}")
def api_update_employer(emp_id: int, data: dict):
    emp = bot_core.update_employer(emp_id, data.get("name"), data.get("admin_id"), data.get("description"), data.get("active"))
    if not emp:
        raise HTTPException(404, "کارفرما یافت نشد")
    return {"ok": True, "employer": emp}

@app.delete("/api/employers/{emp_id}")
def api_delete_employer(emp_id: int):
    if bot_core.delete_employer(emp_id):
        return {"ok": True}
    raise HTTPException(404, "کارفرما یافت نشد")

@app.post("/api/employers/{emp_id}/regenerate")
def api_regenerate_employer_code(emp_id: int):
    code = bot_core.regenerate_employer_code(emp_id)
    if code:
...
.nav-employers .page-header::before,
...
<aside class="sidebar" id="sidebar">
  ...
  <ul class="nav-list" id="nav-list">
    ...
    <li><button class="nav-item" data-page="employers"><span class="nav-icon"><svg viewBox="0 0 20 20" fill="none"><rect x="3" y="4" width="14" height="12" rx="2" stroke="currentColor" stroke-width="1.5" fill="none"/><circle cx="8" cy="9" r="2" stroke="currentColor" stroke-width="1.3" fill="none"/><path d="M4 16c0-2.2 1.8-4 4-4s4 1.8 4 4" stroke="currentColor" stroke-width="1.3" fill="none" stroke-linecap="round"/><path d="M13 8l2 2 3-3" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg></span> <span>کارفرمایان</span></button></li>
...
<main class="main nav-dashboard" id="main-content">
  ...
  <div class="page" id="page-employers">
    <div class="page-header"><div class="page-title">مدیریت کارفرمایان</div><div class="page-subtitle">تعریف و مدیریت کارفرمایان و لینک دعوت اختصاصی</div></div>
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-wrap:wrap;gap:10px;">
      <button class="btn btn-primary" id="btn-add-employer">➕ افزودن کارفرمای جدید</button>
    </div>
...
<div class="modal-overlay" id="employer-modal">
  <div class="modal">
    <div class="modal-title" id="employer-modal-title">افزودن کارفرمای جدید</div>
    <div class="form-group"><label class="form-label">نام کارفرما</label><input class="form-input" id="emp-name" placeholder="مثلاً: شرکت ایران خودرو"></div>
    <div class="form-group"><label class="form-label">آیدی عددی کارفرما در بله</label><input class="form-input" id="emp-admin-id" placeholder="مثلاً: 1682983321"></div>
    <div class="form-group"><label class="form-label">توضیحات (اختیاری)</label><textarea class="form-textarea" id="emp-description" placeholder="توضیحات درباره این کارفرما..." rows="2"></textarea></div>
    <input type="hidden" id="emp-edit-id" value="">
    <div class="modal-actions">
      <button class="btn btn-outline" id="btn-emp-cancel">انصراف</button>
      <button class="btn btn-primary" id="btn-emp-save"><svg viewBox="0 0 14 14" width="13" height="13" style="vertical-align:-2px;margin-left:3px"><path d="M2 2v10h10V4L9 1H3a1 1 0 00-1 1z" stroke="currentColor" stroke-width="1.3" fill="none"/><line x1="7" y1="1" x2="7" y2="8" stroke="currentColor" stroke-width="1.3"/><line x1="4" y1="5" x2="10" y2="5" stroke="currentColor" stroke-width="1.3" opacity=".5"/></svg>ذخیره</button>
    </div>
  </div>
</div>
...
(function() {
  ...
  function navigateTo(page) {
    ...
    var pageClasses = ['nav-dashboard','nav-messages','nav-users','nav-sops','nav-analytics','nav-broadcast','nav-employers'];
  ...
  function loadEmployers() {
    api('/api/employers').then(function(r) {
      var list = $('employers-list');
      if (!r.employers || !r.employers.length) {
        list.innerHTML = '<div class="empty-state"><div class="empty-state-icon"><svg viewBox="0 0 36 36" width="36" height="36"><rect x="7" y="8" width="20" height="20" rx="2.5" stroke="currentColor" stroke-width="2" fill="none" opacity=".25"/><circle cx="14" cy="14" r="3" stroke="currentColor" stroke-width="1.5" fill="none" opacity=".5"/><path d="M8 28c0-3.3 2.7-6 6-6s6 2.7 6 6" stroke="currentColor" stroke-width="1.5" fill="none" opacity=".5"/></svg></div><div>هیچ کارفرمایی تعریف نشده</div><button class="btn btn-primary" style="margin-top:12px" onclick="showEmployerModalAdd()">➕ افزودن کارفرما</button></div>';
        return;
      }
      var html = '';
      for (var i = 0; i < r.employers.length; i++) {
        var emp = r.employers[i];
        var botLink = 'https://bale.me/' + (APP.botUsername || 'botusername') + '?start=emp_' + emp.code;
        var statusColor = emp.active ? 'var(--success)' : 'var(--text-muted)';
        var statusText = emp.active ? 'فعال' : 'غیرفعال';
        html +=
          '<div class="sop-card">' +
            '<div class="sop-header">' +
              '<div class="sop-title" style="color:var(--accent)">#' + emp.id + ' — ' + escHtml(emp.name) + '</div>' +
              '<div class="sop-meta">' +
                '<span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:' + statusColor + ';margin-left:4px;"></span>' +
                statusText + ' | ' + escHtml(emp.created_at) +
              '</div>' +
            '</div>' +
            '<div class="sop-response" style="font-size:13px;">' +
              '<div style="margin-bottom:6px;"><span style="color:var(--text-muted);font-weight:600;">آیدی عددی:</span> ' + escHtml(emp.admin_id) + '</div>' +
              (emp.description ? '<div style="margin-bottom:6px;"><span style="color:var(--text-muted);font-weight:600;">توضیحات:</span> ' + escHtml(emp.description) + '</div>' : '') +
              '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-top:8px;">' +
                '<span style="color:var(--text-muted);font-weight:600;font-size:12px;">🔗 لینک دعوت:</span>' +
                '<code style="direction:ltr;font-size:11px;background:var(--bg-secondary);padding:6px 10px;border-radius:6px;word-break:break-all;flex:1;">' + escHtml(botLink) + '</code>' +
                '<button class="btn btn-sm btn-outline copy-link-btn" data-link="' + escAttr(botLink) + '" style="flex-shrink:0;">📋 کپی</button>' +
              '</div>' +
            '</div>' +
            '<div class="sop-actions">' +
              '<button class="btn btn-outline btn-sm emp-edit-btn" data-id="' + emp.id + '" data-name="' + escAttr(emp.name) + '" data-admin-id="' + escAttr(emp.admin_id) + '" data-desc="' + escAttr(emp.description || '') + '">✏️ ویرایش</button>' +
              '<button class="btn btn-outline btn-sm emp-regenerate-btn" data-id="' + emp.id + '" title="تولید لینک جدید">🔄 لینک جدید</button>' +
              '<button class="btn btn-danger btn-sm emp-del-btn" data-id="' + emp.id + '" data-name="' + escAttr(emp.name) + '">🗑️ حذف</button>' +
            '</div>' +
          '</div>';
      }
      list.innerHTML = html;
    }).catch(function(e) {
      $('employers-list').innerHTML = '<div class="empty-state"><div style="color:var(--danger)">خطا در بارگذاری</div></div>';
    });
  }

  // COPY LINK
  $('employers-list').addEventListener('click', function(e) {
    ...
    if (editBtn) {
      showEmployerModalEdit(
        parseInt(editBtn.getAttribute('data-id')),
        editBtn.getAttribute('data-name'),
        editBtn.getAttribute('data-admin-id'),
    ...
    if (delBtn) {
      var delId = parseInt(delBtn.getAttribute('data-id'));
      var empName = delBtn.getAttribute('data-name') || 'این کارفرما';
      showConfirm('حذف کارفرما', empName, function() {
        return api('/api/employers/' + delId, 'DELETE').then(function() {
          toast('کارفرما حذف شد');
          loadEmployers();
        });
      });
      return;
    }
    var regenBtn = e.target.closest('.emp-regenerate-btn');
    if (regenBtn) {
      var regenId = parseInt(regenBtn.getAttribute('data-id'));
      regenBtn.disabled = true;
      api('/api/employers/' + regenId + '/regenerate', 'POST').then(function(r) {
        toast('✅ لینک جدید تولید شد');
        loadEmployers();
      }).catch(function() {
        toast('خطا', 'error');
      }).finally(function() {
  ...
  function fallbackCopy(text, btn) {
    ...
    try { document.execCommand('copy'); toast('✅ لینک کپی شد'); btn.textContent = '✅ کپی شد'; setTimeout(function() { btn.textContent = '📋 کپی'; }, 2000); } catch(e) { toast('خطا در کپی', 'error'); }
    document.body.removeChild(ta);
  }

  // EMPLOYER MODAL
  function showEmployerModalAdd() {
    $('employer-modal-title').textContent = 'افزودن کارفرمای جدید';
    $('emp-name').value = '';
    $('emp-admin-id').value = '';
    $('emp-description').value = '';
    $('emp-edit-id').value = '';
    $('employer-modal').classList.add('show');
    $('emp-name').focus();
  }

  function showEmployerModalEdit(id, name, adminId, desc) {
    $('employer-modal-title').textContent = 'ویرایش کارفرما';
    $('emp-name').value = name;
    $('emp-admin-id').value = adminId;
    $('emp-description').value = desc;
    $('emp-edit-id').value = id;
    $('employer-modal').classList.add('show');
  }

  $('btn-add-employer').addEventListener('click', showEmployerModalAdd);
  $('btn-emp-cancel').addEventListener('click', function() { $('employer-modal').classList.remove('show'); });
  $('employer-modal').addEventListener('click', function(e) { if (e.target === $('employer-modal')) $('employer-modal').classList.remove('show'); });

  $('btn-emp-save').addEventListener('click', function() {
    var name = $('emp-name').value.trim();
    var adminId = $('emp-admin-id').value.trim();
    var description = $('emp-description').value.trim();
    var editId = $('emp-edit-id').value;
    if (!name || !adminId) { toast('نام و آیدی عددی الزامی است', 'error'); return; }
    $('btn-emp-save').disabled = true;
    var promise;
    if (editId) {
      promise = api('/api/employers/' + editId, 'PUT', { name: name, admin_id: adminId, description: description }).then(function() { toast('کارفرما ویرایش شد'); });
    } else {
      promise = api('/api/employers', 'POST', { name: name, admin_id: adminId, description: description }).then(function() { toast('کارفرما جدید ایجاد شد'); });
    }
    promise.then(function() {
      $('employer-modal').classList.remove('show');
      loadEmployers();
    }).catch(function() {}).finally(function() {
      $('btn-emp-save').disabled = false;
    });
  });
  ...
  function updateStatus() {
    api('/api/status').then(function(s) {
      ...
      if (s.bot_username) APP.botUsername = s.bot_username;
